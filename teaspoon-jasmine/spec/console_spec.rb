  ...
  let(:expected_loading_output) do
    Regexp.new(<<-OUTPUT.strip_heredoc)
      ...
      TypeError: undefined is not a constructor \\(evaluating 'foo\\(\\)'\\)
        # integration/spec_helper(\\.self)?(\\-\\w*)?\\.js:12
    OUTPUT
  end
    ...
    Regexp.new(<<-OUTPUT.strip_heredoc)
      ...
      Failures:
        ...
        2\\) Integration tests allows erroring specs
           Failure/Error: ReferenceError: Can't find variable: foo in http://127\\.0\\.0\\.1:31337/assets/integration/first_spec(\\.self)?(\\-\\w*)?\\.js\\?body=1(\\.js)? \\(line 7\\)
